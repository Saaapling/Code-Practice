package Medium.Maximum_Side_Length_Square;

public class Solution {

    /**
     * Problem: https://leetcode.com/problems/maximum-side-length-of-a-square-with-sum-less-than-or-equal-to-threshold/
     * Time Taken: Unable to come with non-naive solution, 40 min spent thinking + looking at soluton
     * Comments:
     *      - Naive Solution: Loop through all the possible squares, starting with side length = 1,
     *          and stop at the first loop through the array without a valid square
     *          - Can store existing sums in a prefix-sum array to speed up future calculations
     *      - Optimal Solution:
     *          - Prefix Sum + Binary Search
     *          - Rational:
     *              - Prefix Sum: By saving an array where each element (i,j) is the sum of all elements
     *                  where x <= i and y <= j, the  calculation needed to find the sum of values
     *                  where x,y are in the range (i_1,i_2), (j_1,j_2) can be simplified to a small
     *                  calculation that can be inferred (below)
     *              - Binary Search: Binary Search is preferred to iteration from 0/max because for worst
     *                  case scenarios, binary search guarantees log(n) performance, where-as iteration is
     *                  O(n) performance. Moreover, with the prefix sum array, summing large squares is not
     *                  more expensive than small squares
     *      - Implementation Time: 70 min...
     *          - (Could not come up with this solution, thus implementation time rather than total time)
     */

    public int[][] construct_prefix_sum(int[][] mat){
        int[][] prefix_sum = new int[mat.length][mat[0].length];

        prefix_sum[0][0] = mat[0][0];
        for (int i = 1; i < mat.length; i++)
            prefix_sum[i][0] = mat[i][0] + prefix_sum[i-1][0];
        for (int j = 1; j < mat[0].length; j++)
            prefix_sum[0][j] = mat[0][j] + prefix_sum[0][j-1];

        for (int i = 1; i < mat.length; i++)
            for (int j = 1; j < mat[0].length; j++)
                prefix_sum[i][j] = mat[i][j] + prefix_sum[i-1][j] + prefix_sum[i][j-1] - prefix_sum[i-1][j-1];

        return prefix_sum;
    }

    public boolean check_valid(int[][] prefix_sum, int k, int threshold){
        for (int i = 0; i < prefix_sum.length - k; i++){
            for (int j = 0; j < prefix_sum[0].length - k; j++){
                int sum = prefix_sum[i + k][j + k];
                if (i > 0 && j > 0){
                    sum -= (prefix_sum[i-1][j+k] + prefix_sum[i+k][j-1]);
                    sum += prefix_sum[i-1][j-1];
                } else if (i > 0){
                    sum -= prefix_sum[i-1][j+k];
                } else if (j > 0){
                    sum -= prefix_sum[i+k][j-1];
                }
                if (sum <= threshold)
                    return true;
            }
    ...
    public int maxSideLength(int[][] mat, int threshold) {
        int[][] prefix_sum = construct_prefix_sum(mat);
        int low = 0;
        int high = Math.min(mat.length, mat[0].length);

        while (low <= high){
            int mid = (high + low) / 2;
            if (check_valid(prefix_sum, mid, threshold)) {
                low = mid + 1;
            } else{
                high = mid - 1;
            }
